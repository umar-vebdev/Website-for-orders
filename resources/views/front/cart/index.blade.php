...
<div class="overflow-x-hidden">
    ...
    <div class="flex flex-col gap-3 mb-32 max-w-full w-full">

        @foreach($cart as $id => $item)
            <div
                class="flex flex-wrap items-center gap-4 p-3 rounded-2xl
    ...
    <div class="fixed bottom-4 left-0 right-0 z-50 flex gap-2 px-4 sm:px-6">
        <form action="{{ route('cart.clear') }}" method="POST" class="flex-1">
            @csrf
            <button type="submit"
                class="w-full py-3 bg-gradient-to-r from-red-600 to-red-800 hover:from-red-500 hover:to-red-700 text-white text-lg sm:text-xl font-medium rounded-2xl shadow-lg transition">
                Очистить корзину
            </button>
        </form>

        <a href="{{ route('checkout.form') }}" class="flex-1 py-3 flex items-center justify-center
                bg-gradient-to-r from-blue-600 to-blue-800
                hover:from-blue-500 hover:to-blue-700
                text-white text-lg sm:text-xl font-medium rounded-2xl shadow-lg transition">
            Оформить заказ
        </a>
    </div>
