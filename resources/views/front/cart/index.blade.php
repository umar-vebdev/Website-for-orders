...
<div class="max-w-md mx-auto relative px-2">
            ...
            <div class="flex items-center gap-3 p-3 border-b border-white/5 last:border-0 hover:bg-white/[0.04] transition-colors">
                ...
                <div class="flex items-center gap-1 shrink-0">
                    <div class="flex items-center bg-white/5 rounded-full p-0.5 border border-white/10">
                        <form action="{{ route('cart.update', $id) }}" method="POST" class="flex items-center" id="form-{{ $id }}">
                            ...
                            <button type="button" 
                                ...
                                aria-label="��������� ����������" class="w-10 h-10 flex items-center justify-center text-slate-300 active:scale-90 transition-transform disabled:opacity-30">
                            ...
                            <button type="button" 
                                onclick="updateQty('{{ $id }}', {{ $item['quantity'] + 1 }})"
                                aria-label="��������� ����������" class="w-10 h-10 flex items-center justify-center text-accent active:scale-90 transition-transform">
                    ...
                    <form action="{{ route('cart.remove', $id) }}" method="POST" class="shrink-0">
                        @csrf
                        @method('DELETE')
                        <button type="submit" aria-label="������� ����� �� �������" class="w-10 h-10 flex items-center justify-center text-slate-500 hover:text-red-500 transition-colors">
    ...
    <div class="flex flex-col gap-3">
        ...
        <form action="{{ route('cart.clear') }}" method="POST" class="w-full text-center">
            @csrf
            <button type="submit" onclick="return confirm('�������� ������� �������?')" class="text-slate-500 hover:text-red-500 text-[10px] font-black uppercase tracking-[0.2em] transition-colors py-2">
