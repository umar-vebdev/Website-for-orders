...
@section('content')
    ...
    <div class="mb-6 px-2 flex flex-col gap-4">
        <div class="flex justify-between items-center">
            ...
            <form action="{{ route('admin.orders.destroyAll') }}" method="POST"
                onsubmit="return confirm('ВНИМАНИЕ! Это безвозвратно удалит ВСЕ заказы из базы. Продолжить?')">
                @csrf @method('DELETE')
                <button type="submit"
                    class="flex items-center gap-2 px-3 py-2 rounded-xl border border-red-500/30 bg-red-500/5 text-red-500 hover:bg-red-500 hover:text-white transition-all duration-300">
        ...
        <form method="GET" action="{{ route('admin.orders') }}" class="flex flex-col md:flex-row gap-2">
            ...
            <input type="text" name="search" value="{{ request('search') }}"
                placeholder="Поиск: имя, телефон, № заказа..."
                class="flex-1 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-sm placeholder:text-slate-500 focus:outline-none focus:border-accent/50">

            <div class="flex gap-2">
                <input type="date" name="date_from" value="{{ request('date_from') }}"
                    class="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-accent/50">
                <input type="date" name="date_to" value="{{ request('date_to') }}"
                    class="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-accent/50">
            </div>

            <button type="submit"
                class="px-4 py-2 rounded-xl bg-accent text-white text-sm font-bold hover:bg-accent/80 transition-colors">
    ...
    <div id="orders-list" class="flex flex-col gap-3">
        @forelse($orders as $order)
            ...
            <div
                class="glass-panel rounded-2xl p-4 border-white/5 hover:border-accent/20 transition-all duration-300 relative overflow-hidden group">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="flex items-center justify-between gap-2">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-sm font-bold text-white uppercase truncate max-w-[150px]">{{ $order->name }}
                            </h3>
                        </div>
                        <div
                            class="px-2 py-0.5 rounded-md {{ $statusColor }} text-[8px] font-black uppercase tracking-wider shadow-sm">
                    ...
                    <div class="flex items-center justify-between border-t border-white/5 pt-3">
                        <div class="flex flex-col gap-1">
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    class="text-[11px] text-slate-300 font-mono tracking-tighter">{{ $order->phone }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <i class="fas fa-clock text-slate-600 text-[8px]"></i>
                                <span
                                    class="text-[9px] text-slate-500 font-bold uppercase">{{ $order->created_at->format('d.m.Y H:i') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4">
                            <div class="text-right">
                                ...
                                <span
                                    class="text-sm font-display font-black text-white italic tracking-tight">{{ number_format($order->total_price, 0, ',', ' ') }}
                                    ₽</span>
                            </div>
        ...
        @empty
            <div class="py-20 flex flex-col items-center opacity-30">
                ...
                <p class="text-center text-white text-[10px] font-black uppercase tracking-widest">Заказов в этой категории
                    нет</p>
            </div>
        @endforelse
    </div>
